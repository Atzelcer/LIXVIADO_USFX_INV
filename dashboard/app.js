...
async function generarReporte() {
  ...
  </style></head><body><div class="toolbar"><button onclick="window.print()">Imprimir / Guardar como PDF</button><button onclick="window.close()">Cerrar vista</button></div><main class="report"><header class="cover"><div><small>REPORTE TÉCNICO DE PREDICCIÓN</small><h1>Biolix</h1><p>Tratamiento bacteriano de lixiviados</p></div><div class="logos"><img src="${logoTecnologia}" alt="Facultad de Tecnología"><img src="${logoAmbiental}" alt="Ingeniería Ambiental"></div></header><section class="meta"><div><small>FECHA DE EMISIÓN</small><b>${fecha}</b></div><div><small>MODELO UTILIZADO</small><b>${escapar(modeloActivo)}</b></div><div><small>TIPO DE TRATAMIENTO</small><b>${escapar(e.tipo_tratamiento)}</b></div></section><div class="content"><section class="section"><div class="section-title"><b>1</b><h2>Caracterización inicial del lixiviado</h2></div><table><tbody>${filasLixiviado}</tbody></table></section><section class="section"><div class="section-title"><b>2</b><h2>Configuración bacteriana</h2></div><div class="bacteria-layout">${bacteriasReporte}</div></section><section class="section"><div class="section-title"><b>3</b><h2>Condiciones del reactor</h2></div><table><tbody>${condiciones}</tbody></table></section><section class="section"><div class="section-title"><b>4</b><h2>Resultados estimados</h2></div><table class="results"><thead><tr><th>Variable</th><th>Inicial</th><th>Final estimado</th><th>Cambio</th><th>Unidad</th></tr></thead><tbody>${resultados}</tbody></table></section><div class="disclaimer"><b>Nota:</b> Este reporte documenta una predicción generada por el modelo y debe conservarse junto con los registros experimentales correspondientes.</div><div class="signature"><div>Responsable del análisis</div><div>Responsable del laboratorio</div></div></div></main></body></html>`;
